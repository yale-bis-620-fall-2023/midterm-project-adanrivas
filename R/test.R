...
test_that("get_study_conditions correctly joins studies and conditions data", {
  ...
  expected <- data.frame(nct_id = c("NCT001", "NCT002"),
...
test_that("summarize_study_conditions", {
  # Define a simple data frame of study conditions
  study_conditions_df <- data.frame(
    condition_name = c('Condition1', 'Condition2', 'Condition3', 'Condition1', 'Condition2', 'Condition3'))
  ...
  # Check that the "n" column contains only non-negative integers
  expect_true(all(result$n >= 0))
  expect_true(is.numeric(result$n))
  
  # Check that the "condition_name" column contains only factor levels
  expect_true(is.factor(result$condition_name))
})
...
test_that("plot_study_conditions_histogram", {
  ...
  result <- plot_study_conditions_histogram(studies_df, top_n = 2)
